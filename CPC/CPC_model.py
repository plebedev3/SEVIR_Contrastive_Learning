...
class CDCK6(nn.Module):
    ...
    def __init__(self, timestep, batch_size, seq_len, input_channels, img_width, img_height, device):
        ...
        self.encoder = nn.Sequential( # downsampling factor = 160
            nn.Conv2d(2, 512, kernel_size=10, stride=5, padding=3, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, kernel_size=8, stride=4, padding=2, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, x_reverse, hidden1, hidden2):
        ...
        t_samples = torch.randint(2, size=(1,)).long() # randomly pick time stamps. ONLY DO THIS ONCE FOR BOTH GRU.
        #z = z.view((self.batch_size, self.seq_len, 512))
        # first gru
        # input sequence is N*C*L, e.g. 8*1*20480
        x = x.view((self.batch_size*self.seq_len, self.input_channels, self.img_width,self.img_height))
        #print(x.size())
        z = self.encoder(x)
        z = z.view((self.batch_size, self.seq_len, 512))
...
#         forward_seq = z[:,:t_samples+1,:] # e.g. size 8*100*512
        output1, hidden1 = self.gru1(z, hidden1) # output size e.g. 8*100*256
        c_t = output1[:,t_samples,:].view(batch, 128) # c_t e.g. size 8*256
        pred = torch.empty((self.timestep,batch,512)).float().to(self.device) # e.g. size 12*8*512
        for i in np.arange(0, self.timestep):
            linear = self.Wk1[i]
            pred[i] = linear(c_t) # Wk*c_t e.g. size 8*512
        for i in np.arange(0, self.timestep):
            total = torch.mm(z[i], torch.transpose(pred[i],0,1)) # e.g. size 8*8
            correct1 = torch.sum(torch.eq(torch.argmax(self.softmax(total), dim=0), torch.arange(0, batch).to(self.device))) # correct is a tensor
            nce += torch.sum(torch.diag(self.lsoftmax(total))) # nce is a tensor

        # second gru
        # input sequence is N*C*L, e.g. 8*1*20480
        x_reverse = x_reverse.view((self.batch_size*self.seq_len, self.input_channels, self.img_width,self.img_height))
        ...
        #     encode_samples[i-1] = z[:,t_samples+i,:].view(batch,512) # z_tk e.g. size 8*512
        # @forward_seq = z[:,:t_samples+1,:] # e.g. size 8*100*512
        output2, hidden2 = self.gru2(z, hidden2) # output size e.g. 8*100*256
        ...
        for i in np.arange(0, self.timestep):
            total = torch.mm(z[i], torch.transpose(pred[i],0,1)) # e.g. size 8*8
            correct2 = torch.sum(torch.eq(torch.argmax(self.softmax(total), dim=0), torch.arange(0, batch).to(self.device))) # correct is a tensor
            nce += torch.sum(torch.diag(self.lsoftmax(total))) # nce is a tensor
...
class CDCK5(nn.Module):
    ...
    def forward(self, x, hidden):
        batch = x.size()[0]
        t_samples = torch.randint(2, size=(1,)).long() # randomly pick time stamps
        # input sequence is N*C*L, e.g. 8*1*20480
        z = self.encoder(x)
        # encoded sequence is N*C*L, e.g. 8*512*128
        # reshape to N*L*C for GRU, e.g. 8*128*512
...
class CDCK2(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        nce = 0 # average over timestep and batch
        encode_samples = z #torch.empty((self.timestep,batch,512)).float() # e.g. size 12*8*512
#         for i in np.arange(1, self.timestep+1):
#             encode_samples[i-1] = z[:,t_samples+i,:].view(batch,512) # z_tk e.g. size 8*512
#         forward_seq = z[:,:t_samples+1,:] # e.g. size 8*100*512
        output, hidden = self.gru(z, hidden) # output size e.g. 8*100*256
